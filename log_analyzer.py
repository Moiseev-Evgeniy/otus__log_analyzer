...
def event_logging(message: str, level: str, data: dict) -> None:
    """Event logging function."""

    if config["SELF_LOG_DIR"]:
        write_log_to_file(message, level, data)
    else:
        getattr(log, level)(message, **data)
    getattr(log, level)(message, **data)


def write_log_to_file(message: str, level: str, info: dict) -> None:
    ...
    with open(ROOT_DIR / config["SELF_LOG_DIR"] / self_log_file_name, "a", encoding="utf-8") as log_file:
        data = {"datetime": datetime.now().ctime(), "level": level, "message": message}
        data.update(info)
        json.dump(data, log_file)
        log_file.write("\n")


def update_config(external_config_path: str) -> None:
...
def make_report(file_name: str) -> list[dict] | None:
...
def create_report_file(file_date: str, reporting_data: list[dict]) -> None:
...
def main():
    ...
    if not file_names:
        event_logging("There are no log files", "info", {"dir": str(ROOT_DIR / config["LOG_DIR"])})
        return
    ...
    if reporting_data := make_report(file_name):
        create_report_file(file_date, reporting_data)

    event_logging("The report has been generated", "info", {})


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="config.json", help="path from root dir to config.json")
    ...
    try:
        ...
        event_logging("Unexpected error", "error", format_exc_info(None, None, {"exc_info": True}))
